...
const VISION_SYSTEM =
  ...
  'JSON 格式：{"items":[{"name":"食物名称","kcal":数字,"unit":"/小包|/块|/100g等","category":"零食|食堂|外卖|自制|饮品|水果","confidence":0到100,"note":"规格或补充说明","box":[x,y,w,h]}]}' +
  '要求：1) name 为图片中该食物区域的主要文字；2) kcal 为该食物每份的千卡数字（只保留数字）；' +
  '3) confidence 是识别置信度；4) box 为该食物区域在整图中的归一化位置 [x,y,w,h]（0~1 的小数，尽量精确，不确定可省略）；' +
  '5) 如果图片无法识别出任何食物，返回 {"items":[]}。';
